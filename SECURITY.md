...
Report via [GitHub Security Advisory](https://github.com/9t29zhmwdh-coder/BugRadar/security/advisories/new)
...
- **Two AI backends, and the choice decides what leaves the machine.** Ollama is the default and runs against `localhost:11434`, so nothing leaves the device. Selecting Claude sends the incident summary and the surrounding log lines to `api.anthropic.com` using your own API key. Both are a setting, not a build option
